...
<div class="easyui-layout" style="width: 100%;height: 100%;">
	...
	<table class="table_cx h_set" cellpadding="0" cellspacing="0">
		<tbody>
			<tr>
				...
				<td>
					<select style="width:150px" id="dateType${id}" name="dateType" class="easyui-combobox">
						<c:if test="${id != 'yw4'}">
						<option value="1" <c:if test="${param.dateType == '1'}"> selected</c:if>>在院时间</option>
						</c:if>
						<option value="2" <c:if test="${param.dateType == '2'}"> selected</c:if>>出院时间</option>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<select id="yymd" name="usePurpose" class="easyui-combobox" style="width:150px" >
						<option value="">----不限----</option>
						<option value="1" <c:if test="${param.usePurpose == '1'}"> selected</c:if>>治疗</option>
						<option value="2" <c:if test="${param.usePurpose == '2'}"> selected</c:if>>预防</option>
						<option value="3" <c:if test="${param.usePurpose == '3'}"> selected</c:if>>治疗+预防</option>
					</select>
				</td>
			</tr>
			</c:if>
			...
			<tr>
				...
				<td>
					<select id="sjlb" name="state" style="width:150px" class="easyui-combobox">
						<option value="1" <c:if test="${param.state == '1'}"> selected</c:if>>病原学送检</option>
						<option value="2" <c:if test="${param.state == '2'}"> selected</c:if>>血培养送检</option>
					</select>
				</td>
			</tr>
			</c:if>
	...
	<script type="text/javascript">
		function query${id}() {
			...
			if($('#dateType${id}').combobox('getValue') == '2' && '${id}' != 'yw4'){
				$('#searchform${id}').attr("action","${reportUrl}${reportFile}_CY.cpt&__bypagesize__=${__bypagesize__}");
			}
