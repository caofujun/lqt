			...
			<tr>
				...
				<td style="width:37%;">
					<table class="table_childer">
						<tr>
							<td class="rightTextAlign vat" style="width:90px;">报卡类别：</td>
							<td>${cs.reportType}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat" >中暑诊断：</td>
							<td>${cs.sunstrokeDiagnosis}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat">患者情况：</td>
							<td>${cs.patientStatus}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat">中暑日期：</td>
							<td><fmt:formatDate value="${cs.sunstrokeDt}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate></td>
						</tr>
					</table>
				</td>
...
<script>
 ...
 function bkDetail(zyid,mzid,msid){
	 if(zyid){
		 parent.menuInfo.clickMenu('高温中暑报卡详情','/cdc/f_view/sunstrokeReport.shtml?zyid='+zyid+'&masterid='+msid,true);
	 }else if(mzid){
		 parent.menuInfo.clickMenu('高温中暑报卡详情','/cdc/f_view/sunstrokeReport.shtml?mzid='+mzid+'&masterid='+msid,true);
	 }
 }
</script>