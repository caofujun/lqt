			...
			<tr>
				...
				<td style="width:37%;">
					<table class="table_childer">
						<tr>
							<td class="rightTextAlign vat" style="width:90px;">诊断：</td>
							<td colspan="3">${cs.diagnose}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat">首次诊断时间：</td>
							<td colspan="3"><fmt:formatDate value="${cs.diagnosedDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
						</tr>
						<tr>
							<td class="rightTextAlign vat" >临床分期：</td>
							<td colspan="3">${cs.clinicalStages}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat">病理学类型：</td>
							<td>${cs.pathologyName}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat">确诊时期别：</td>
							<td colspan="3">T&nbsp;${cs.timeT}&nbsp;N&nbsp;${cs.timeN}&nbsp;M&nbsp;${cs.timeM}</td>
						</tr>
					</table>
				</td>
				...
				<td style="text-align: center;">
					...
					<div class="topSpace" style="word-break:break-all;"><fmt:formatDate value="${cs.validdt}" type="both"/></div>
				</td>				
			</tr>
...
<script>
 ...
 function bkDetail(zyid,mzid,msid){
	 if(zyid){
		 parent.menuInfo.clickMenu('肿瘤病例报卡详情','/cdc/f_view/tumourReport.shtml?zyid='+zyid+'&masterid='+msid,true);
	 }else if(mzid){
		 parent.menuInfo.clickMenu('肿瘤病例报卡详情','/cdc/f_view/tumourReport.shtml?mzid='+mzid+'&masterid='+msid,true);
	 }
 }
</script>