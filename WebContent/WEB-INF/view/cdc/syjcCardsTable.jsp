			...
			<table class="infos" cellpadding="0" cellspacing="0" style="width: 100%;">
				<tr>
					<td colspan="11" style="background-color: #fff;border-top: 1px solid #ddd;background-color: #eee;">
						...
						<span class="toolBar">
						<a class="btn_icon" title="导出Excel" onclick="toExcel('${cs.masterid}');"><i class="icon iconfont">&#xe628;</i></a>
				...
				<tr>
					...
					<td style="width:41%;">
						<table class="table_childer">
							<tr>
								<td class="rightTextAlign vat" style="width:100px;">发病时间：</td>
								<td><fmt:formatDate value="${cs.startDate}" type="both"/></td>
								<td class="rightTextAlign vat" style="width:80px;">就诊时间：</td>
								<td><fmt:formatDate value="${cs.diagnoseDate}" type="both"/></td>
							</tr>
							<tr>
								<td class="rightTextAlign vat" >初步诊断：</td>
								<td colspan="3">${cs.initdiagnosis}</td>
							</tr>
							<tr>
								<td class="rightTextAlign vat" >既往病史：</td>
								<td colspan="3">${cs.previoushistory}</td>
							</tr>
							<tr>
								<td class="rightTextAlign vat">是否使用抗生素：</td>
								<td colspan="3">${cs.isusedantibiotic}${!empty antibiotic?"("+antibiotic+")":""}</td>
							</tr>
							<tr>
								<td class="rightTextAlign vat">备注：</td>
								<td colspan="3">${cs.notes}</td>
							</tr>
						</table>
					</td>
					...
					<td style="text-align: center;">
						...
						<div class="topSpace" style="word-break:break-all;"><fmt:formatDate value="${cs.validDate}" type="both"/></div>
					</td>					
				</tr>
...
$(function(){
 ...
 function bkDetail(zyid,mzid,msid){
	 if(zyid){
		 parent.menuInfo.clickMenu('食源监测报卡详情','/cdc/f_view/fsmReport.shtml?zyid='+zyid+'&masterid='+msid,true);
	 }else if(mzid){
		 parent.menuInfo.clickMenu('食源监测报卡详情','/cdc/f_view/fsmReport.shtml?mzid='+mzid+'&masterid='+msid,true);
	 }
 }
 function toExcel(mid){
	 if(mid){
		var form=$("<form>");//定义一个form表单
		form.attr("style","display:none");
		form.attr("target","");
		form.attr("method","post");
		form.attr("action","${webroot}/cdc/export/file/toExcel.shtml");
		var input1=$("<input>");
		input1.attr("type","hidden");
		input1.attr("name","masterid");
