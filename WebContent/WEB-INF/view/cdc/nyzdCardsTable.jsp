			...
			<tr>
				<td colspan="11" style="background-color: #fff;border-top: 1px solid #ddd;background-color: #eee;">
					...
					<span class="toolBar">
						...
						<a class="btn_icon" href="javascript:;" onclick="sendMessage('${cs.zyid}','${cs.mzid}');" title="干预消息"><i class="icon iconfont">&#xe63e;</i></a>			
			...
			<tr>
				...
				<td style="width:37%;">
					<table class="table_childer">
						<tr>
							<td class="rightTextAlign vat" style="width:90px;">中毒数量：</td>
							<td>${cs.poisoningNum}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat" >中毒农药名称：</td>
							<td>${cs.pesticideList}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat">中毒原因：</td>
							<td>${cs.poisoningReason}</td>
						</tr>
						<tr>
							<td class="rightTextAlign vat">诊断日期：</td>
							<td><fmt:formatDate value="${cs.diagnoseDt}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate></td>
						</tr>
						<tr>
...
<script>
 ...
 function bkDetail(zyid,mzid,msid){
	 if(zyid){
		 parent.menuInfo.clickMenu('农药中毒报卡详情','/cdc/f_view/nyzdReport.shtml?zyid='+zyid+'&masterid='+msid,true);
	 }else if(mzid){
		 parent.menuInfo.clickMenu('农药中毒报卡详情','/cdc/f_view/nyzdReport.shtml?mzid='+mzid+'&masterid='+msid,true);
	 }
 }
</script>