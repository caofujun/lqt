				...
				<tr>
					...
					<td style="width:40%;">
						<table class="table_childer">
							...
							<tr>
								<td class="rightTextAlign vat" >主要体征：</td>
								<td>${cs.sign}</td>
							</tr>
							<tr>
								<td class="rightTextAlign vat" >主要诊断：</td>
								<td>${cs.mainDiagnosis}</td>
							</tr>
							<tr>
								<td class="rightTextAlign vat">可疑原因：</td>
								<td>${cs.pathogenesis}</td>
							</tr>
						</table>
					</td>
					<td class="BothSidesSpace topBottomSpace"  style="width:1%"><div class="borderStyle"></div></td>
...
<script>
 ...
 function bkDetail(zyid,mzid,msid){
	 if(zyid){
		 parent.menuInfo.clickMenu('食源异常报卡详情','/cdc/f_view/fsaReport.shtml?zyid='+zyid+'&masterid='+msid,true);
	 }else if(mzid){
		 parent.menuInfo.clickMenu('食源异常报卡详情','/cdc/f_view/fsaReport.shtml?mzid='+mzid+'&masterid='+msid,true);
	 }
 }
</script>