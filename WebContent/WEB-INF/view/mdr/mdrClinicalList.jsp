				...
				<table class="table_cx" cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							...
							<td>
								<select style="width:120px" id="dateType" class="easyui-combobox"><option value="s">送检日期</option><option value="j">检出日期</option></select>
							</td>
						</tr>
						<tr>
							...
							<td>
								...
								<c:if test="${empty testOrderNo}">
									value="${startDate}"
								</c:if> style="width:108px"  class="Wdate text" onclick="WdatePicker()"  />
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
							<input type="text" class="auto-tip" style="width:108px" data-tip="菌名称" id="lispathoName"/>
							</td>
						</tr>
						<tr><td height="5"></td><td></td></tr>
						<tr>
						...
						<tr>
							...
							<td>
								<div class="select_del">
									...
									<!-- <select id="dnlx" class="easyui-combobox" data-options="multiple:true,editable:false,onLoadSuccess:function(data){$('#dnlx').combobox('clear')}" style="width:120px">
										<option value="0">敏感</option>
										<option value="1">MDR</option>
										<option value="XDR">XDR</option>
										<option value="PDR">PDR</option>
									</select>
									<a href="javascript:void(0)" title="清除" class="select_img_del" onclick="$('#dnlx').combo('clear');"></a> -->
								</div>
							</td>
						</tr>
						<tr><td height="5"></td><td></td></tr>
...
// 								var resPropName = '';
// 								return resPropName += '<a href="#" class="easyui-tooltip" id="id_tooltip_' + r.testOrderNo + '_'+ r.pathoCode+'">干预 &nbsp;&nbsp;<span class="arrow_down"></span></a>';
// 							}}  
		                ]
	                ],	                	
	                pagination:true,
...
/*                 var byt = $('#byt').combogrid('getValue');
				...
				var url = "${webroot}/xn011Dclymx/f_json/exportExcelMdr.shtml?key="+key+"&specDescribes="+specDescribes+"&infectTypeId="+infectTypeId+"&esbl="+esbl+"&patientState="+patientState+"&dateType="
						+dateType+"&startDate="+startDate+"&endDate="+endDate+"&deptType="+deptType+"&deptId="+deptId+"&isAudited="+isAudited+"&searchString="+searchString+"&qc="+qc+"&jszd="+jszd+"&rsId="+rsId+"&gx="+gx+"&gl="+gl+"&lispathoName="+lispathoName;
		...
		function opts(divId, deptId, defContent, testOrderNo,dt,zyid,surveyDeptId,orderno,infectTypeId,normItemId,normOrderno,resprop) {
			...
			if(!v){
				...
				viewMdr.intervOpts(testOrderNo,dt,zyid,surveyDeptId,orderno,v,n,normItemId,normOrderno,resprop,defContent);
			}
		}
		</script>
	</body>
