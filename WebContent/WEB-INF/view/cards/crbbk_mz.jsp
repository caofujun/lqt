...
<style type="text/css">
	...
	.piece{border: 1px solid #ddd;padding: 5px;padding-bottom:10px; width: initial;margin: 0px;}
	.pieceTitle{padding:20px 5px 5px 0px;}
...
<body style="width: auto; min-width: 1100px;">
		...
		<div style="width: 100%;">
			...
			<div class="card_cont">
				<div class="card_cont_h">
					...
					<input type="hidden" name="masterid" id="masterid" value="${mz.masterid}" />
					<input type="hidden" name="cardid" id="cardid" value="${mz.cardid}" />
					<input type="hidden" name="isemptycard" id="isemptycard" 
					<c:if test="${! empty BRXX}">
						value="${BRXX.isemptycard}" 
					</c:if>
					<c:if test="${! empty mz}">
						value="${mz.isemptycard}" 
					</c:if>
					/>
				</div>			
				<table class="mainTable" title="患者基本信息" >
					<c:if test="${BRXX.isemptycard==1 || mz.isemptycard==1}">
					<tr>
					...
					<tr>
							...
							<td>
								<select style="width: 142px;" id="reporttypeid" name="reporttypeid" class="easyui-combobox">
									<option value="1" <c:if test="${mz.reporttypeid=='1'}"> selected="selected"</c:if> >初次报告</option>
									<option value="2" <c:if test="${mz.reporttypeid=='2'}"> selected="selected"</c:if> >订正报告</option>
									<option value="3" <c:if test="${mz.reporttypeid=='3'}"> selected="selected"</c:if> >死亡报告</option>
								</select>
					...
					<tr>
						...
						<td class=""><input type="text" name="patientId" <c:if test="${empty isEmptyCard}">readonly="readonly"</c:if> required="true" class="easyui-validatebox" style="width: 130px;" value='<c:if test="${! empty BRXX}">${BRXX.patientId}</c:if><c:if test="${! empty mz}">${mz.patientId}</c:if>'/></td>
						<td class="rightTextAlign"><span class="red">*</span>门诊号/${patientZyTitle}：</td>
						<td><input type="text" style="width: 130px;" id="mzid" name="mzid" <c:if test="${empty isEmptyCard}">readonly="readonly"</c:if> required="true" class="easyui-validatebox" value="<c:if test="${! empty BRXX}">${BRXX.mzid}</c:if><c:if test="${! empty mz}">${mz.mzid}</c:if>"/></td>
						<td class="rightTextAlign"><span class="red">*</span>就诊次数：</td>
						<td><input type="text" style="width: 130px;" <c:if test="${empty isEmptyCard}">readonly="readonly"</c:if> required="true" class="easyui-validatebox" name="visitId" onkeyup="this.value=this.value.replace(/\D/g,'')" value="<c:if test="${! empty BRXX}">${BRXX.visitId}</c:if><c:if test="${! empty mz}">${mz.visitId}</c:if>"/></td>
					</tr>
					<tr>
						<td class="rightTextAlign"><span class="red">*</span>患者姓名：</td>
						<td><input type="text" style="width: 130px;" <c:if test="${empty isEmptyCard}">readonly="readonly"</c:if> required="true" class="easyui-validatebox" name="patientName" value="<c:if test="${! empty BRXX}">${BRXX.patientName}</c:if><c:if test="${! empty mz}">${mz.patientName}</c:if>"/></td>
						<td class="rightTextAlign">患儿家长姓名：</td>
						<td><input type="text" style="width: 130px;" name="parentName" id="parentName"
						<c:choose>
							<c:when test="${!empty mz}">
								value="${mz.parentName}"
							</c:when>
							<c:otherwise>
								value="${BRXX.parentName}"
							</c:otherwise>
						</c:choose>
						/></td>
						<td>&nbsp;</td>
					...
					<tr>
						...
						<td>	
							<select style="width: 142px;" name="idtype" id="idtype" class="easyui-combobox" >
								<c:forEach items="${cardType}" var="ct">
									<option value="${ct.dictCode}" <c:if test="${mz.idtype == ct.dictCode}">selected="selected"</c:if>>${ct.dictName}</option>
								</c:forEach>
							</select>
							<input type="hidden" name="idtypevalue" id="idtypevalue" />
						</td>
						...
						<td class="rightTextAlign NCNPlace" style="display: none;">国家传染病编号：</td>
						<td class="NCNPlace" style="display: none;"><input type="text" id="nidn" name="nidn" value="${mz.nidn}"/></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<select style="width: 142px;" name="sexid" id="sexid" class="easyui-combobox" >
								<option value="1" 
								<c:choose>
							...
							<input type="hidden" name="sexname" id="sexname" value="${mz.sexname}"/>
						</td>
						<td class="rightTextAlign"><span class="red">*</span>出生日期：</td>
						<td><input type="text" style="width: 130px;" id="birthday" readonly="readonly" name="birtyday" required="true" 
						<c:choose>
							<c:when test="${!empty mz}">
								value='<fmt:formatDate value="${mz.birtyday}" pattern="yyyy-MM-dd"/>'
							</c:when>
							<c:otherwise>
								value='<fmt:formatDate value="${BRXX.birthday}" pattern="yyyy-MM-dd"/>'
							</c:otherwise>
						</c:choose>  class="Wdate text easyui-validatebox" onchange="$('#age').val(ages($(this).val()));$('#ageunit').val('岁')" onclick='WdatePicker({dateFmt:"yyyy-MM-dd",maxDate:"<fmt:formatDate value="${now}"/>"})' /></td>
						
						<td class="rightTextAlign"><span class="red">*</span>生日不详填年龄：</td>
						<td>
							<input type="text" style="width: 60px;" id="age" name="age" required="true" class="easyui-validatebox" onkeyup="value=value.replace(/[^\d]/g,'')"
							<c:choose>
								<c:when test="${!empty mz}">
									value="${mz.age}"
								</c:when>
							...
							<select id="ageunit" style="width: 60px;" name="ageunit" class="easyui-combobox">
								<option value="岁"
								<c:choose>
					...
					<tr>
						...
						<td colspan="5">
							<input type="text" id="unit" name="unit" style="width: 93%;" 
							<c:choose>
								<c:when test="${!empty mz}">
									value="${mz.unit}"
								</c:when>
					...
					<tr>
						...
						<td>
							<input type="text" id="telp" name="telp" style="width: 130px;" required="true" class="easyui-validatebox" onkeyup="value=value.replace(/[^\d]/g,'')"
								<c:choose>
									<c:when test="${!empty mz}">
										value="${mz.telp}"
									</c:when>
					...
					<%-- <tr>
						<td class="rightTextAlign"><span class="red">*</span>病人属于：</td>
						<td>
							<select style="width: 142px;" id="belong" name="areatypeid" onchange="CRBMaddress();" >
								<c:forEach items="${patientBelong}" var="pb">
									<option value="${pb.dictCode}" <c:if test="${mz.areatypeid==pb.dictCode}">selected="selected"</c:if> >${pb.dictName}</option>
								</c:forEach>
							</select>
							<input type="hidden" name="areatypename" id="areatypename" value="${mz.areatypename}"/>
						</td>
						<td class="rightTextAlign"><span class="red">*</span>现住地址国标：</td>
						<td>
							<span id="nowAddrCode"></span>
						</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
					</tr> --%>
					<tr>
						<td>&nbsp;</td>
						<td>
							<select style="width: 142px;" name="province" id="sheng" class="easyui-combobox">
									<option></option>
							</select>
							省
						</td>
						<td>&nbsp;</td>
						<td>
							<select style="width: 142px;" name="city" id="shi" class="easyui-combobox">
									<option></option>
							</select>
							市
						</td>
						<td>&nbsp;</td>
						<td>
							<select style="width: 142px;" name="country" id="xian" class="easyui-combobox">
									<option></option>
							</select>
							县（区）
						</td>
					...
					<tr>
						...
						<td colspan="5">
							<input type="text" style="width: 93%;" id="address_info" name="addr" value="${mz.addr}"/>
					...
					<tr>
						...
						<td>
							<select style="width: 142px;" id="professionid" name="professionid" class="easyui-validatebox easyui-combobox" >
									<option></option>
									<c:forEach items="${groupClassify}" var="gc">
										<option value="${gc.dictCode}" <c:if test="${mz.professionid==gc.dictCode}">selected="selected"</c:if> >${gc.dictName}</option>
									</c:forEach>
							</select>
						</td>
					...
					<tr>
						...
						<td>
							<input type="text" class="Wdate text" id="death_time" readonly="readonly" name="deaddate" value='<fmt:formatDate value="${mz.deaddate}" type="both"/>' style="width: 130px;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'<fmt:formatDate value="${now}" type="both"/>'})" />
						</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
			...
			<div class="card_cont">
				<div class="card_cont_h">
					...
					<div class="card_c_h_btn">
						<a class="btn_icon" href="javascript:void(0)" onclick="addDisease()" title="添加传染病记录"><i class="icon iconfont">&#xe665;</i></a>
					</div>
				</div>
				<div class="byt_table">			
			...
			<div class="card_cont">
				...
				<div id="fkTabs" style="width:100%;" class="easyui-tabs" title="附卡信息"></div>
			</div>
			...
			<div class="card_cont">
				...
				<table class="mainTable">
					<tr>
						...
						<td>
							<div class="select_del"><input class="easyui-combobox" name="reportdoctorid" id="reportdoctorid" style="width: 130px;" value="${mz.reportdoctorid}" /></div>
							<input type="hidden" name="reportdoctorname" id="reportdoctorname" value="${mz.reportdoctorname}"/>
						</td>
						<td class="rightTextAlign"><span class="red">*</span>上报科室：</td>
						<td>
							<div class="select_del"><input name="reportdeptid" id="reportdeptid" style="width: 130px;"  /></div>
					...
					<tr>
						...
						<td class="fillDateTime" style="display: none;"><input type="text" name="filldate" id="filldate" class="Wdate text" style="width: 130px;" 
						<c:choose>
							<c:when test="${!empty mz}">
						...
						</c:choose> onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'<fmt:formatDate value="${now}" type="both"/>'})" /></td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
	...
	<div id="chooseDisease" >
		<div id="distool">
			<input id="diseaseKey" type="text" style="width: 100%;height:28px;" class="easyui-textbox" data-options="prompt:'输入疾病编号/疾病名称/助记码检索'" />
		</div>
	...
	$(function(){
		...
		<c:choose>
			...
			<c:when test="${!empty BRXX.presentaddr}">
				setTimeout('$("#address_info").val("${BRXX.presentaddr}")',1000);
			</c:when>
		</c:choose>
		if($("#birthday").val()){
			ages($("#birthday").val());
	   ...
	   $("#reportdoctorid").combogrid("setValue","${mz.reportdoctorid}");
	   $("#reportdeptid").combogrid("setValue","${mz.reportdeptid}");
	...
	function addDiseaseRow(id,name){
		...
		var singleRow = "<tr rowno="+id+" >"+
							...
							"<td><input type='text' name='diseaseList["+rowcount+"].startdate' readonly='readonly' class='Wdate text fbdate easyui-validatebox' data-options='required:true' id='fbdate_"+id+"' style='width: 130px;' onclick=\"WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'<fmt:formatDate value="${now}"/>'})\" /></td>"+
							"<td><input type='text' name='diseaseList["+rowcount+"].diagnosedate' readonly='readonly' class='Wdate text zddate easyui-validatebox' data-options='required:true' id='zddate_"+id+"' style='width: 130px;' onclick=\"WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'<fmt:formatDate value="${now}"/>'})\" /></td>"+
							"<td><select style='width: 120px;' id='ct1_id_"+id+"' name='diseaseList["+rowcount+"].casetypeid' class='blfl1 fillName easyui-combobox' data-options='required:true,onChange:function(n,o){fillName(\"ct1_id_"+id+"\",\"ct1_name_"+id+"\")}' >"+
								"<option></option>"+	
							...
							"</td>"+
							"<td><select style='width: 80px;' id='ct2_id_"+id+"' name='diseaseList["+rowcount+"].casetypeid2' class='blfl2 fillName easyui-combobox ' data-options='required:true,onChange:function(n,o){fillName(\"ct2_id_"+id+"\",\"ct2_name_"+id+"\")}' >"+
							...
							"</td>"+
							"<td><input type='text' style='width: 120px;' class='notes' id='note_"+id+"' name='diseaseList["+rowcount+"].diseasenotes'/></td>"+
	...
	function fixDiseaseRow(orgdiseaseid,newid,newname){
		...
		if(fjhfkDids.indexOf(newid)>=0){
			$.messager.show({ title: '提示', msg: '附卡提示：该疾病需要填写肺结核相关附卡。'});
			isFJHFKExist = true;
		    addTab("肺结核转诊/推荐单","${webroot}/cdc/f_view/fjhzzdfk.shtml");
		}
