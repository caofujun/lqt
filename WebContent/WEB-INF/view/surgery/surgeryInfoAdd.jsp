	    ...
	    <div class="card_cont">
			...
			<table class="info_table">
			    <tbody>  
			    	<tr>
			    		<th>选择患者：</th>
			    		<td colspan="9"><input type="text" id="id_sel_patient" style="width: 130px;" /></td>
			    	</tr>
			        <tr>
			            ...
			            <th>姓名:</th>
			            <td>
			            	<input type="text" style="width: 88px;" id="patientName" name="patientName"  />
			            </td>
			            <th>性别:</th>
			            <td>
			            	<nis:radio name="sex" dictcode="sex" defvalue="男"/>
			            </td>
			            <th>年龄:</th>
			            <td>
			            	<input type="text" style="width: 38px;" id="age" name="age" />
							<nis:select name="ageUnit" id="ageUnit" cssCls="easyui-combobox" dictcode="age_unit" exp="style=\"width: 45px;\"" />
			            </td>
			            <th>床号:</th>
			            <td>
			            	<input type="text" style="width: 88px;" name="bedNo" id="bedNo" />
			            </td>
			        </tr>
			        <tr>
			        	<th>科室:</th>
			            ...
			            <th>入院时间:</th>
			            <td>
			            	<input type="text" class="Wdate text" name="inHospAt" id="inHospAt" style="width: 88px;" onclick="WdatePicker()" />
			            </td>
			            <th>出院时间:</th>
			            <td>
			            	<input type="text" class="Wdate text" name="outAt" id="outAt" style="width: 88px;" onclick="WdatePicker()" />
			            </td>
			            <th>卡片状态:</th>
			            <td>
			    ...
			    <tbody>  
			        <tr>
			            ...
			            <td>
			            	...
			            	<input type="text" id="id_operName" name="operName" class="easyui-validatebox" style="width:150px;"  value="${st005Ssxxb.operName}"/>
	...
	<div  data-options="region:'south',border:false">
		<div style="padding:10px 0px; text-align:center;">	
			<div class="footer_btn">
				<div class="n_btn_blue">
					<a href="javascript:void(0);" onclick="$('#id_form').submit();"  class="no_ico"><span>保存</span></a>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		window.setTimeout(function(){
			Comm.form({
				id: 'id_form',
				url: '${webroot}/st005Ssxxb/f_view/surgeryInfoAdd.shtml',
				//subbtn: 'btn_save',
				onLoading : function () {
					$.messager.progress({
						text : '正在提交中....',
				...
				success : function(json) {
					...
					if (json.result === 'success') {
                		...
                		parent.menuInfo.clickMenu('手术信息','/st005Ssxxb/f_view/toSurgeryInfoEdit.shtml?'+json.data, true, null, null, '${param.tabBodyId}');
    				} else {
    					$.messager.show({title : '提示',msg : json.msg});
    				}
				}
		...
		$('#id_sel_patient').combogrid({
	        ...
	        columns:[
	        	[
	             ...
	             {field:'visitId',title:'住院次数',hidden:true},
	             {field:'patientId',title:'编号',hidden:true},
	             ...
	             {field:'outAt',title:'出院时间',sortable:true,width:80}
	            ]
	        ],
			onClickRow : function(index,row){
				...
				$('#bedNo').val(row.bedNo);
				if("${patientZyValue}" == "patientId"){
					$('#tempID').html(row.patientId);
				}else{
					$('#tempID').html(row.zyid);
				}
				$('#patientName').val(row.patientName);
				$('.sex[value='+row.sex+']').attr("checked",'checked');
				$('#age').val(row.age);
				$("#ageUnit option[value='"+row.ageUnit+"']").attr("selected","selected");
				if(row.inHospAt){
					$("#inHospAt").val(row.inHospAt.substring(0,10));
				}
				if(row.outAt){
					$("#outAt").val(row.outAt.substring(0,10));
				}
			}
		});
		
		
		$('#id_operName').combogrid({
			delay: 1000,    
		    mode: 'remote',
		    loadMsg : '正在查询中...',
		    value: '',
	        ...
	        fitColumns: true,
	        textField:'operName',
			url: '${webroot}/zg011Ss/f_json/query.shtml?page=1&size=20',
	        columns:[
	        	[
	             {field:'operaCnname',title:'手术名称',sortable:true,width:120}
	            ]
	        ]
		});
