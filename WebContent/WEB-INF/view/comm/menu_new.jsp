...
<div data-options="region:'west',split:false,collapsed:false" title="" style="width: 76px;">
	<nav class="main_menu">
		<ul id="nav">
			...
			<c:choose>
			<c:when test="${menu.menuNo=='J'}">
			<li id="menu_${st.index+1}"  >
				<a href="javascript:;" onclick="menuInfo.refreshMain('${menu.menuName}','${webroot}${menu.destUrl}','${menu.menuId}')">
					 <i class="icon iconfont">${menu.image}</i> 
					<img src="${webroot}/resources/images_org/nis/menu/home.png">
			...
			<c:when test="${menu.destUrl!=null}">
				<li id="menu_${st.index+1}" >
				<a href="javascript:;"  onclick="javascript:clickThirdMenu('${menu.menuName}','${menu.destUrl}',true,'${menu.menuNo}','${menu.menuNo}')">
			...
			<c:otherwise>
			<li id="menu_${st.index+1}" >
				<a href="javascript:;"  onmouseover="showmenu('menu_${st.index+1}','${menu.menuNo}');">
					 <i class="icon nisfont">${menu.image}</i>					
					<span class="nav_text">${menu.menuName}</span>
				</a>				
			</li>
...
<div>
	...
	<c:if test="${menu.menuNo==cldMenu.parentMenuNo && cldMenu.ownership=='hospital' && cldMenu.menuType=='0'}">	
		<li class="${cldMenu.menuNo}">			
			<c:choose>
				<c:when test="${cldMenu.destUrl!=null}"><a href="javascript:;"  class="has_sub_second_nav" onclick="javascript:clickThirdMenu('${cldMenu.menuName}','${cldMenu.destUrl}',true,'${cldMenu.menuNo}','${cldMenu.menuNo}')"><span class="subnav_text" >${cldMenu.menuName}</span></a></c:when>
				<c:otherwise><a href="javascript:;"  class="has_sub_second_nav"><span class="subnav_text">${cldMenu.menuName}</span><i class="icon iconfont">&#xe65b;</i></a></c:otherwise>
			</c:choose>			
			<ul class="third_menu">
	...
	var menuInfo = {
			...
			clickMenu : function(name, url, isReload, menuNo, urlPrefix, fromTabBodyId, position) {
				...
				if(url.indexOf('http://')== -1 && url.indexOf('https://')== -1) {
					...
					url = webroot + url;
					url += 'ownership=hospital&tabBodyId=' + _tabId+'&menuNo='+menuNo;
				...
				if(!$('#mainTabs').tabs('exists', name)){
				    $('#mainTabs').tabs('add', {
				    	title: name,
		                content: '<iframe name="'+_tabId+'" menuNo="' + menuNo + '" id="'+_tabId+'" width="100%" height="100%" frameborder="0" src="'+url+'" scrolling="auto"></iframe>',
		                closable: true
				    });
				}else{				
			...
			//获取当前选中的tab
			getCurSelectTab : function () {
				return $('#mainTabs').tabs('getSelected');
			},
			...
			//打开帮助文档
			openHelpWin : function() {
				var currTab = this.getCurSelectTab();
				var tbIframe = currTab.find('iframe:first-child'); 
				var currMenuNo = tbIframe.attr('menuNo');
				var reportFile = $(window.frames[tbIframe.attr('id')].document).find("#reportFile").val();
				if(typeof(reportFile)!='undefined'){
					...
					if (currMenuNo) {
						//判断有没有设置帮助文档
							$.ajax({
							url : '${webroot}/moduleExplain/f_json/queryHasHelp.shtml',
							dataType: 'json',
							data: {mid:currMenuNo},
							type: 'post',
							cache: false,
							...
							success: function(json) {
								if(json.result === 'success') {
							            ...
							            //打开帮助窗口
										Comm.dialogGlobal({
								        	url:"${webroot}/moduleExplain/f_view/showHelp.shtml?mid="+currMenuNo,
						    		...
						    		$.messager.show({ title: '提示', msg: '当前页面没有配置帮助页面！'});
						    		return;
