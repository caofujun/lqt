...
Showbo.Msg={  
    ...
    show:function(cfg){  
      ...
      if(cfg.wait){  
        ...
        if(cfg.msg && cfg.ctimer) {this.dvCT.innerHTML=cfg.msg+(cfg.ctimer--) + 's'+ '<div class="clear"></div>';} else {this.dvCT.innerHTML+=cfg.msg+'<div class="clear"></div>';};  
      ...
      this.onResize(false);
      if (cfg.ctimer) {this.cp = cfg; this.run();}
    },  
    hide:function(){  
    ...
    run : function (cfg) {
		if (this.cp.ctimer <= 0) {
			...
			this.dvCT.innerHTML=this.cp.msg+(this.cp.ctimer--) + 's'+ '<div class="clear"></div>';
			this.timer = setInterval("Showbo.Msg.run()",1000);
		}
    }
