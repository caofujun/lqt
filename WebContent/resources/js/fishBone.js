$.fn.fishBone = function(data) {
    ...
    function creataFishBone(data) {
        ...
        $(data).each(function(index) {
            ...
            $.each(this, function(name, value) {
                if (name.indexOf('pz')>-1) {
                	var a = name.substring(2);
                	for(var i=0;i<value.length;i++){ 
                		var round = "";
                		var pdca = "";
                		var qs="";
	                	if(value[i].pzStatus == '0'||value[i].pzStatus == null ||value[i].pzStatus == 'null'){
	                		round = "<b class='round red'></b>";
	                	}else if(value[i].pzStatus == '1'){
	                		round = "<b class='round gray'></b>";
	                	}else if(value[i].pzStatus == '2'){
	                		round = "<b class='round green'></b>";
	                	}else if(value[i].pzStatus == '3'|| value[i].pzStatus == '4'||value[i].pzStatus == '5'){
	                		round = "<b class='round orange'></b>";
	                	}
	                	if(value[i].pdcaStatus>0){
	                		pdca = "<c onmouseover='lookzy(this,\""+value[i].pzId+"\",\"pdca\")' onmouseout='closeTip()'>P</c>";
	                	}
	                	if(value[i].qsStatus>0){
	                		qs = "<c onmouseover='lookzy(this,\""+value[i].pzId+"\",\"qs\")' onmouseout='closeTip()' style='background-color:"+value[i].fxColor+"'>D</c>";
	                	}
	                    /*var li = $("<li><a>"  + value + "</a></li>").css("border-left","1px solid "+color);*/
	                	var li = $("<li id='li_"+a+"_"+i+"' onclick='nOnClickEvent(\""+a+"\",\""+i+"\",\""+value[i].pzId+"\",\""+value[i].pzStatus+"\")'><a title='"+value[i].zbName+"'>" +round+ value[i].zbName +"&nbsp;&nbsp;"+pdca+qs+"</a></li>").css("border-left","1px solid #43a6da");
	                	li.appendTo(ul);
                	} 
                }
            });
          //封装审理时间和案号
            if(itemclass=="bottom"){
                ...
                $.each(this, function(name, value) {
                    if (name == 'charDate') {
                        var li = $("<li class='line-first'>" + value + "</li>")
                        .css('background-position', "0px "+(parseInt(lineFirstY)-33)+"px");//30是原计算结果的偏移量
                        li.appendTo(ul);
                        return;
                    }
                });
