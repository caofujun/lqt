...
var Comm = {
		...
		mask:function(dialogId,msg){
			$("#"+dialogId).css("position","relative");
			var msgHtml = "数据处理中.....";
			if(msg){
				msgHtml = msg;
			}
			var _width = $("#"+dialogId).width();
			var _height = $("#"+dialogId).height();
			var _html = '<div id="_comm_mask" style="position:absolute;z-index:90000;width:'+_width+'px;height:'+_height+'px;background-color:#ccc; top:0; left:0; -moz-opacity:0.6; filter:alpha(opacity=60); filter:Alpha(opacity=80); opacity: .60;">';
			_html +='<div style="margin:100px auto;width:160px;height:30px;line-height:26px;"><span><div style="width:30px;height:30px;background:url('+webroot+'/resources/images/indicator.gif) no-repeat right;float:left;"></div></span><span style="margin-left:10px;color:red;">'+msgHtml+'</span></div>';
			_html +='</div>';
			$("#"+dialogId).append(_html);
		},
		unmask:function(){
			$("#_comm_mask").remove();
		},
		...
		dialog : function(attr) {
			...
			if(attr.url) {
				...
				if(attr.url.indexOf('?')!=-1)
					...
					_param = '?dialogId='+_dialogId;
				if(attr.type==='iframe') {
					attr.content = ['<iframe id="iframe_',_dialogId,'" src="" width="100%" height="100%" scrolling="auto" frameborder="0"></iframe>'].join('');
					openDialog(attr);
			...
			function openDialog(attr) {
				...
				_dialog.dialog({
					...
					modal: (typeof(attr.modal) == "undefined"? true : attr.modal),
					resizable:false,
					collapsible:(typeof(attr.collapsible) == "undefined"? false : attr.collapsible),
					maximizable:false,
					top : (typeof(attr.top) == "undefined"? null : attr.top),
			...
			return {'iframeId':'iframe_'+_dialogId};
		},
...
Comm.util = {
        ...
        */
		...
		showLoading : function(id, msg) {
			...
			$("#" + id).html('<div id="' + id + '_loading" style="margin-top: 35px;margin-left: 80px;"><img src="${webroot}/resources/images/loading.gif"/><span>' + msg + '...</span></div>');  
			//document.getElementById(id). innerHTML('<div id="' + id + '_loading" style="margin-top: 35px;margin-left: 80px;"><img src="${webroot}/resources/images/loading.gif"/><span>' + msg + '...</span></div>');
		},
		//移除加载中效果
...
Comm.phone = {
		...
		newDownload : function() {
        	parent.Comm.dialog({title: '温馨提示',
				content:['<div style="padding: 20px;"><p>该功能需要浏览器控件支持，请下载安装</p>',
				         '<p style="text-align: center;"><input type="button" class="btn" value="下载" onclick="location = \'',webroot,'/nyTellPlugin/plugin/nytel.exe\'"></p></div>'].join(''),
				width:300, height:160
			});
		},
		//新控件呼叫
		newCall : function(attr){
			//缓存操作对象
			Comm.setObjectCache(attr.cachObject);
			Comm.phone.patientId = attr.patientId;
			Comm.phone.patientName = attr.patientName;
			//调用电话处理
			NYTellOptions.tellHandle(attr.tellObject, attr.phone);
		},
		//点击呼叫电话
						...
						Comm.dialog({
							...
							closeFn: function() {
								if(window.attachEvent) {
									...
									try {
										var tell = parent.window.top.JYTopTele;
										tell.tellControl.StopPcRecord();
									} catch (e) {
		...
		//通过患者id 查询该患者所有录音记录
		playByPid: function(id) {
			$.ajax({
		        url: webroot+'/foTelrecord/f_view/findByPId.shtml',
		        type: 'post',
		        data: {pid:id},
		        dataType: 'json',
		        success : function(obj) {
					if(obj!=null&&obj.length>0) {
						...
						var _cont = ['<div style="padding: 20px 20px 10px 20px;"><div id="',_embedId,'Div"></div>'];
						_cont.push('<table class="table" border="0" cellpadding="0" cellspacing="1" style="margin:0px; width:100%;"><tbody>');
						_fileName = obj[0].fileName;
						_cont.push('<caption style="font-weight: bold; padding: 5px;">',obj[0].visitName,'的通话录音</caption>');
						_cont.push('<tr class="title_check"><td class="t_title_l" style="width: 35%;">随访人</td><td class="t_title_l" style="width: 45%;">录音日期</td><td class="t_title_l" style="width: 20%;">播放</td></tr>');
						
						for (var i = 0; i < obj.length; i++) {
							_cont.push('<tr><td>',obj[i].userName,'</td><td>',obj[i].startTime,'</td><td><a href="javascript:Comm.phone.playEmbed(\'',_embedId,'\',\'',obj[i].fileName,'\')" class="ico_deal" title="播放"></a></td></tr>');
						}
			...
			$.ajax({
		        ...
		        success : function(obj) {
					if(obj!=null) {
						...
						_cont.push('<caption style="font-weight: bold; padding: 5px;">',obj.visitName,'的通话录音</caption>');
						_cont.push('<tr class="title_check"><td class="t_title_l" style="width: 35%;">录音人</td><td class="t_title_l" style="width: 45%;">录音日期</td><td class="t_title_l" style="width: 20%;">播放</td></tr>');
						_cont.push('<tr><td>',obj.realName,'</td><td>',obj.startTime,'</td><td><a href="javascript:Comm.phone.playEmbed(\'',_embedId,'\',\'',obj.fileName,'\')" class="ico_deal" title="播放"></a></td></tr>');
						_cont.push('</tbody></table>');
						_cont.push('</div>');
						Comm.dialog({
							title: '播放录音',
...
//extend validatebox
$.fn.combogrid.defaults=$.extend({}, $.fn.combogrid.defaults, {height:26});
$.fn.combotree.defaults=$.extend({}, $.fn.combotree.defaults, {height:26});
$.fn.combobox.defaults=$.extend({}, $.fn.combobox.defaults, {height:26});
$.fn.textbox.defaults=$.extend({}, $.fn.textbox.defaults, {height:24});
//tab设置title拓展
...
var emptyView = $.extend({},$.fn.datagrid.defaults.view,{
	onAfterRender:function(target){
		...
		if (!$(target).datagrid('getRows').length){
			var d = $('<div class="datagrid-empty" style="color:#1c4587"></div>').html(opts.emptyMsg || '暂无数据').appendTo(vc);
			d.css({
				position:'absolute',
				left:0,
				top:50,
...
$.extend($.fn.validatebox.defaults.rules, {
	...
	phone : {
        validator : function(value) {
        	var qu = value.substring(0,4);
            var mobile = value.substring(4);
            value = qu+'-'+mobile;
...
Comm.widget = {
		...
		initTime : function(panel, selId, btimeId, etimeId, initSelVal) {
			...
			_panel.append(['<select id="',selId,'" onchange="Comm.widget.timeChange(\'',selId,'\',\'',btimeId,'\',\'',etimeId,'\')">',
					...
					'<input type="text" id="',btimeId,'" class="Wdate text" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" style="width: 100px;"/>',
					' ~ <input type="text" id="',etimeId,'" class="Wdate text" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" style="width: 100px;"/>',
					'</span>'].join(''));
...
//获取当前地址栏的地址 add by xiaobin
function getRootPath_web() {
    //获取当前网址，如： https://localhost:8080/csmcmt/share/meun.jsp
    var curWwwPath = window.top.document.URL;
    //获取主机地址之后的目录，如： csmcmt/share/meun.jsp
