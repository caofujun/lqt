...
//		bindTreeEvents(target);
		...
		function appendNodes(ul, children, depth){
			for(var i=0; i<children.length; i++){
				...
				var formateValue = null;
				try {
					formateValue = item.attributes.formateValue;
				} catch (e) {
				}
				if(formateValue){
					$('<span class="tree-title"></span>').html(formateValue).appendTo(node);
				}else{
...
(function($){
	...
	$.messager = {
		...
		alert: function(title, msg, icon, fn) {
			var content = '<div class="mt20">' + msg + '</div>';
			switch(icon) {
				case 'error':
...
(function($){
	 ...
	 */
	function wrapTabs(container,_csm_tab_expand) {
		var tabs = $.data(container, 'tabs').tabs;
		var cc = $(container);
		cc.addClass('tabs-container');
		cc.wrapInner('<div class="tabs-panels"/>');
		var _csm_html = '';
		if(_csm_tab_expand._csm_expand){
			_csm_html = '<ul class="tabs" id="'+_csm_tab_expand._csm_expand+'"></ul>';
		}else{
			_csm_html = '<ul class="tabs"></ul>';
		}
	...
	$.fn.tabs = function(options, param){
		...
		return this.each(function(){
			...
			if (state) {
				...
				var _csm_tab_expand =  $.parser.parseOptions(this, [
				                                 			'width','height','tools',
				                                			{fit:'boolean',border:'boolean',plain:'boolean'}
				                                		]);
				wrapTabs(this,_csm_tab_expand);
			}
...
//		state.validating = false;
	...
	function validate(target){
		var state = $.data(target, 'validatebox');
		//yuejing: add if(state===undefined) return true;
	...
	function fixColumnSize(target, field){
		...
		function fix(field){
			...
			for(var i=0,len=rules.length; i<len; i++){
				var rule = rules[i];
				if (typeof col.cellSelector == 'undefined') continue;
